  ...
  auth
    ...
    .then(UC => {
      ...
      sendEmailVerification(user).then(() => {
        Alert.alert('Verify Email', '/An email will be sent shortly to verify your account.')
      })
    })
    .catch(error => {
...
const store_user = user => {
  ...
  if (user) {
      ...
      .then(documentSnapshot => {
        ...
        if (documentSnapshot.exists) {
          return Alert.alert(
            ...
            'It seems that your selected username already has an account',
          );
        } else {
          return setDoc(doc_ref, user_refrence).then(() => {
    ...
    console.log(
      `no user info was retrieved, please try again - ${Date().toLowerCase()}`,
    );
  }
};
  ...
  if (email.length < 1) {
    ...
    sendPasswordResetEmail(auth, email)
      .then(() => {
        ...
        Alert.alert(
          'Successfull',
          'Please check your email, click back button for login screen',
        );
        console.log('100.000%');
      })
      .catch(error => {
        ...
        Alert.alert(
          'Problem',
          'It seems we dont have a user with the email ' + email,
        );
        // ..
      });
  }
...
const get_storage_image = name => {
  getDownloadURL(ref(storageDB, name))
  .then(url => {
      console.log('collecting image download url')
      _image = url;
 ...
 * @param document_id
 */
const user_delete_request = async (document_id, image) => {
  const imageRef = ref(storageDB, `${image}`);

  deleteObject(imageRef)
    .then(() => {
      console.log(`deleted file ${imageRef}`);
    })
    .catch(error => {
...
const user_delete_account = async user_name => {
  await deleteDoc(doc(db, 'users', `${user_name}`))
    .then(() => {
